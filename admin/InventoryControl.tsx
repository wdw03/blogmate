...
const InventoryControl: React.FC<InventoryControlProps> = ({ domains, onRefresh }) => {
  ...
  // Detailed form state for single add
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col xl:flex-row items-center justify-between gap-6 bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm">
        <div className="flex flex-wrap items-center gap-4">
           <button 
             ...
             className="flex items-center gap-2.5 bg-blue-600 text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-xl shadow-blue-500/20 hover:bg-blue-700 transition-all hover:-translate-y-0.5 active:scale-95"
           >
             <Plus size={16} strokeWidth={3} /> Add_New_Asset
           </button>
           <button 
             onClick={() => setShowBulkModal(true)}
             className="flex items-center gap-2.5 bg-slate-900 text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-slate-800 transition-all shadow-lg"
           >
        ...
        <div className="relative w-full xl:w-96 group">
           ...
           <input 
             ...
             className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-4 pl-14 pr-6 text-[11px] font-bold text-slate-900 focus:outline-none focus:bg-white focus:border-blue-500 transition-all shadow-inner"
           />
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-[3rem] overflow-hidden shadow-sm">
         <div className="p-10 border-b border-slate-100 bg-slate-50/30 flex items-center justify-between">
            <div>
              <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight italic">Inventory_Manifest</h3>
              <div className="flex items-center gap-2 mt-1">
                 <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></div>
                 <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{filteredDomains.length} Nodes Synchronized</span>
              </div>
            </div>
         </div>
            ...
            <table className="w-full text-left">
               <thead>
                  <tr className="bg-white border-b border-slate-100">
                     <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Asset_Node</th>
                     <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">SEO_Forensics</th>
                     <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Pricing_Module</th>
                    ...
                    <tr key={d.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors group">
                       <td className="px-10 py-6">
                          <div className="flex flex-col">
                             <span className="font-black text-slate-950 text-base group-hover:text-blue-600 transition-colors">{d.domain}</span>
                             <div className="flex items-center gap-2 mt-1">
                       ...
                       <td className="px-10 py-6">
                          <div className="flex gap-2">
                             <div className="px-2 py-1 bg-blue-50 text-blue-600 border border-blue-100 rounded-lg text-[9px] font-black">DA {d.da}</div>
                             <div className="px-2 py-1 bg-indigo-50 text-indigo-600 border border-indigo-100 rounded-lg text-[9px] font-black">DR {d.dr}</div>
                             <div className="px-2 py-1 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-lg text-[9px] font-black">{d.traffic}</div>
                          </div>
                       </td>
                       <td className="px-10 py-6">
                          <div className="flex flex-col gap-1">
                            <div className="flex justify-between items-center text-[10px] font-bold text-slate-400 min-w-[100px]">
                                <span>Guest:</span>
                       ...
                       <td className="px-10 py-6 text-right">
                          <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                             <button className="p-2.5 bg-white border border-slate-200 text-slate-400 rounded-xl hover:text-blue-600 transition-all shadow-sm"><Edit3 size={16} /></button>
                             <button onClick={() => handleDelete(d.id, d.domain)} className="p-2.5 bg-white border border-slate-200 text-slate-400 rounded-xl hover:text-rose-500 transition-all shadow-sm"><Trash2 size={16} /></button>
                          </div>
                       </td>
                    </tr>
                  ))}
          ...
          >
            <header className="px-10 py-8 border-b border-slate-100 bg-white flex items-center justify-between shrink-0">
               <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tighter italic leading-none">Deploy_Asset_Node</h3>
                  </div>
               </div>
            ...
            <footer className="px-10 py-8 border-t border-slate-100 bg-white flex items-center justify-end gap-4 shrink-0">
               ...
               <button type="submit" disabled={isSubmitting} className="px-14 py-5 bg-slate-950 text-white rounded-2xl font-black text-[12px] uppercase tracking-[0.3em] flex items-center gap-4 active:scale-95 disabled:opacity-50">
                  {isSubmitting ? <Loader2 size={18} className="animate-spin" /> : <><Database size={18} fill="currentColor" className="text-blue-400" /> Initialize Node</>}
               </button>
            </footer>
          </form>
        </div>
