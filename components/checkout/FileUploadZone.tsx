        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between px-1">
                <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Article / Content File</span>
                </div>
                <span className="bg-blue-50 text-blue-600 text-[8px] font-black px-2 py-0.5 rounded uppercase tracking-widest border border-blue-100">ALL FILE TYPES SUPPORTED</span>
            </div>

            <div className={`relative group border-2 border-dashed rounded-[2rem] p-12 transition-all duration-300 ${fileUrl ? 'bg-emerald-50/20 border-emerald-300' : (error ? 'bg-rose-50 border-rose-200' : 'bg-slate-50/50 border-slate-200 hover:border-blue-600')
                }`}>
                <input
                    type="file"
                    ref={fileInputRef}
                ...
                <div className="relative z-10 flex flex-col items-center">
                    {uploading ? (
                        <div className="flex flex-col items-center gap-4">
                            ...
                            <h4 className="text-sm font-black text-slate-900 uppercase tracking-tighter italic animate-pulse">Uploading File...</h4>
                        </div>
                    ) : fileUrl ? (
                        <div className="flex flex-col items-center">
                            ...
                            <h4 className="text-base font-black text-slate-900 uppercase tracking-tighter mt-5">File Uploaded Successfully</h4>
                            ...
                            <button onClick={(e) => { e.stopPropagation(); onUpload(''); }} className="mt-4 text-[9px] font-black text-rose-500 uppercase tracking-widest hover:underline">Replace / Remove File</button>
                        </div>
                    ) : (
                        <div className="flex flex-col items-center text-center">
                            ...
                            <h4 className="text-2xl font-black text-slate-950 uppercase tracking-tighter mb-2 italic leading-none">Upload Files</h4>
                            <p className="text-[9px] text-slate-400 font-black uppercase tracking-[0.2em] mb-4">DRAG & DROP OR BROWSE (DOCX, PDF, TXT, MD, GOOGLE DOCS, ETC.)</p>
                            {error && (
