                        ...
                        {!isGatewayActive ? (
                          <div className="space-y-12 animate-in fade-in slide-in-from-left-4 duration-500">
                              ...
                              <div className="space-y-6">
                                  ...
                                  <button 
                                    onClick={() => setIsGatewayActive(true)}
                                    className="w-full py-7 bg-slate-950 text-white rounded-3xl font-black uppercase text-[12px] tracking-[0.4em] flex items-center justify-center gap-4 hover:bg-blue-600 transition-all shadow-2xl active:scale-95"
                                  >
...
const GatewayOption = ({ label, icon, active, selected, onClick, status, isNew }: any) => (
    <button 
        disabled={!active}
        onClick={onClick}
        className={`relative aspect-square w-full rounded-[2.5rem] flex flex-col items-center justify-center gap-6 transition-all border-2 group ${
            selected 
    ...
    >
        ...
        <div className={`w-20 h-20 rounded-[2rem] flex items-center justify-center transition-all duration-500 ${
            active ? (selected ? 'bg-slate-950 text-blue-400 shadow-xl' : 'bg-white text-slate-400') : 'bg-slate-200'
        }`}>
